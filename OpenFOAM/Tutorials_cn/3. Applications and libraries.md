...
在执行时，*wmake会在**Make*目录的$WM_OPTIONS*子目录（*例如* *Make/linuxGccDPInt64Opt* ）中构建一个具有*.dep*文件扩展名的依赖项列表文件，*例如* 我们示例中的*newApp.C.dep*。如果用户希望删除这些文件，*例如* 在进行代码更改后，用户可以运行*wclean* 通过键入脚本：
...
*pisoFoam*仅包含*pisoFoam.C*源代码，可执行文件与所有标准应用程序一样写入$ *FOAM_APPBIN目录。**Make/files*因此包含：
